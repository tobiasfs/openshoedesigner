...
//	std::wstring szComParams;
	...
	m_hIDComDev = CreateFile(tempport.data(), GENERIC_READ | GENERIC_WRITE, 0,
...
//	szComParams = tempport + ":" + std::to_wstring(nBaud) + std::wstring(",n,8,1");
	...
	if (!SetCommState(m_hIDComDev,
			&dcb) || !SetupComm(m_hIDComDev, 10000, 10000) || m_OverlappedRead.hEvent
...
size_t SerialPort::SendData(const char *buffer, size_t size) {
#ifdef __WIN
	if (m_hIDComDev == NULL)
		return (0);
	...
	size_t dwBytesWritten = write(fd, buffer, size);
	return (dwBytesWritten);
#endif
}
...
size_t SerialPort::SendData(std::string data) {
	...
	if (!Opened)
		return 0;
	size_t dwBytesWritten = write(fd, data.c_str(), data.size());
	return (dwBytesWritten);
#endif
}

size_t SerialPort::ReadDataWaiting() {
#ifdef __WIN
	if (m_hIDComDev == NULL)
		return (0);
	...
	//	std::cerr << buffer_WR << ' ' << buffer_RD << '\n';
	return ((buffer_WR + BUFFER_LEN) - buffer_RD) % BUFFER_LEN;
#endif
}

size_t SerialPort::ReadData(char *buffer, size_t limit) {
#ifdef __WIN
