...
BooleanBox::BooleanBox(const BooleanBox &other) {
	...
	if (bufferSizeIntersect <= nx)
		bufferSizeIntersect = nx + 1;
	if (bufferSizeIntersect <= ny)
		bufferSizeIntersect = ny + 1;
	if (bufferSizeIntersect <= nz)
		bufferSizeIntersect = nz + 1;
	...
	if (bufferSizeOccupied < (nx * ny * nz))
		bufferSizeOccupied = nx * ny * nz;
...
void BooleanBox::NormalizeMatrix() {
	// TODO: Implement this.
...
BooleanBox& BooleanBox::operator -=(const BooleanBox &rhs) {
	...
	const auto mx = rhs.matrix[12] - matrix[12];
	const auto my = rhs.matrix[13] - matrix[13];
	const auto mz = rhs.matrix[14] - matrix[14];

	const float eps = (float) 1e-6; // = 1 um
	...
	for (unsigned int m = 0; m <= rhs.nx; m++) {
		...
		if (nx + 1 >= bufferSizeIntersect)
			SizeUpIntersect(nx + 11);
	...
	for (unsigned int m = 0; m <= rhs.ny; m++) {
		...
		if (ry < pY[n] + eps)
			continue;
		if (ny + 1 >= bufferSizeIntersect)
			SizeUpIntersect(ny + 11);
	...
	for (unsigned int m = 0; m <= rhs.nz; m++) {
		...
		if (rz < pZ[n] + eps)
			continue;
		if (nz + 1 >= bufferSizeIntersect)
			SizeUpIntersect(nz + 11);
