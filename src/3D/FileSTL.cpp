...
void FileSTL::WriteStream(const Geometry &geo) {
	...
	for (size_t idx = 0; idx < geo.TriangleCount(); ++idx) {
		...
		col |= ((uint16_t) (floor(tri.c.b * 32.0))) & 31;
		col |= (((uint16_t) (floor(tri.c.g * 32.0))) & 31) << 5;
		col |= (((uint16_t) (floor(tri.c.r * 32.0))) & 31) << 10;
