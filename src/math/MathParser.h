...
public:
	...
	/**\name Code handling
	 *
	 * Parsing code into instructions for a virtual machine.
	 *
...
public:
	...
	/**\brief Single shot static function to convert a string to a double number
	 *
	 * This function might throw an exception, if the expression cannot be
	 * parsed.
	 */
	static double SIFromString(const std::string &expression);
...
public:
	...
	public:
		...
		bool ToBool() const;
		std::string ToString(int digitsAfterComma = -1) const;
	 ...
	 *
	 * Virtual machine for running the calculations. The machine contains a
	 * heap for all the variables (main memory) and a stack for intermediate
	 * results and return values.
	 */
	class VM {
	public:
		/** \brief OpCodes for the virtual machine.
		 *
		...
		enum class OpCode {
			...
			STOP, ///< Stop execution, the VM can only be restarted by calling Reset();
			PUSH, ///< Push one value (with unit) directly onto the stack.
			POP, ///< Pop one value from the stack and discard of it.
			SWAP, ///< Swap the top two values on the stack.
			DUP, ///< Duplicate the top-most value on the stack.
			FETCH, ///< Fetch the value of a variable from the variable-vector and push it onto the stack.
			STORE, ///< Store the top-most value into the variable-vector. The value is left on the stack.
			FETCH_EXT, ///< Fetch the value of an external variable from the variable-vector and push it onto the stack.
			STORE_EXT, ///< Store the top-most value into an external variable. The value is left on the stack.
			...
			DIV, ///< Divide the two to values. The units of the values do not have to be of the same type (e.g. 60V / 3A = 2Ohm).
			...
			NEG, ///< Invert the top value on the stack, i.e. -15cm are changed to 15cm, 7mol to -7mol.
			F_ABS, ///< Calculate the absolute value. The sign is made positive.
			F_EXP, ///< Calculate e to the power of the topmost stack value. The value has to be unitless.
			F_EXP2, ///< Calculate 2 to the power of the topmost stack value. The value has to be unitless.
			F_LOG, ///< Calculate the logarithm to the base of e. The value on the stack has to be unitless.
			F_LOG2, ///< Calculate the logarithm to the base of 2. The value on the stack has to be unitless.
			F_LOG10, ///< Calculate the logarithm to the base of 10. The value on the stack has to be unitless.
			F_MAX, ///< Find the maximum of the top two stack values. The units of the values have to be of the same type.
			F_MIN, ///< Find the minimum of the top two stack values. The units of the values have to be of the same type.
			F_SIN, ///< Calculate the sinus from radians. The value on the stack has to be unitless.
			F_COS, ///< Calculate the cosinus from radians. The value on the stack has to be unitless.
			F_TAN, ///< Calculate the tangens from radians. The value on the stack has to be unitless.
			F_ASIN, ///< Calculate the inverse sinus. The value on the stack has to be unitless.
			F_ACOS, ///< Calculate the inverse cosinus. The value on the stack has to be unitless.
			F_ATAN, ///< Calculate the inverse tangens. The value on the stack has to be unitless.
			F_ATAN2, ///< Calculate the angle in radians from two values (same units). The units of the values have to be of the same type.
		 ...
		 * \name Variables
		 *
		 * Setup and manipulate variables inside the virtual machine running the
		 * code.
		 *
		 * Variables can be converted to external variables if a shared variable
		 * space for several virtual machines is needed.
		 *
		 * The core concept is the Value. A double number with an attached Unit.
		 *
		 ...
		 */
		class Heap: public std::vector<Variable> {
		public:
			size_t Set(const std::string &variablename, const Value &value);
			void Set(const size_t idx, const Value &value);
			size_t GetIndex(const std::string &variablename) const;
			const Variable& Get(const std::string &variablename) const;
			bool Has(const std::string &variablename) const;
	...
	public:
		...
		void Clear(); ///< Remove all variables and code.

		/** \brief Resets the virtual machine
		 *
		 *  Reset and initialize the VM at the beginning of a compiled program.
		 */
		void Reset();
		...
		std::string GetInstruction(size_t idx) const;

		/**\brief Convert instructions to access an internal variable to
		 * instructions addressing external variables.
		 *
		 * \param idxInternal Index in the array of internal variables
		 * \param idxExternal Index in the array of external variables
		 * \return Bool, if at least one instruction was changed
		 */
		bool ConvertToExternal(size_t idxInternal, size_t idxExternal);
	...
	public:
		...
		size_t stepCount = 0;
		size_t maxSteps = (size_t) -1;

	private:
		size_t instructionpointer = 0;
	};

	VM vm; ///< Main virtual Machine

	/**\name Functions for the recursive-descent parser
	 * \{
	 */
private:
	void ParseStatement();
	...
	void AddInstruction(VM::OpCode opcode, const Value &value = Value(0.0),
			size_t idx = 0);
