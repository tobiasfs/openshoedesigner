...
MatlabFile::Version MatlabFile::GetVersion() const {
	return version;
}
	...
	case Version::V4: {
		...
		do {
			...
			if (x > 0) {
				...
				tempchar.resize(x + 1);
				tempchar[x] = 0;
				res = fread(tempchar.data(), sizeof(char), x, fhd);
				M->SetVariableName(std::string(tempchar.data()));
				if (res == 0)
					break;
			}
			...
			if (matrixname.empty()
				...
				fseek(fhd,
						(long int) S1 * (long int) S2
								* (long int) sizeof(double) - sizeof(uint32_t),
						SEEK_CUR);
...
//				fwrite("MI", sizeof(char), 2, fhd); // Big Endian System
	...
	case Version::V4: {
		...
		const uint32_t S1 = M.Size(0);
		const uint32_t S2 = M.Size(1);
